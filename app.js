...
let amountElm = document.querySelector('#amount')
...
btnElm.addEventListener('click' , function(e) {
  ...
  let amountRand = Math.floor(Math.random()*dummyText.length)

  if (amountVal < 1 || amountVal > 9 || isNaN(amountVal)) {

    resultElm.innerHTML = dummyText[amountRand]

  } else {
    let sliceElm = dummyText.slice(0 , amountVal)

    let sliceElmPrag = sliceElm.map(function(item) {
      return '<p>' + item + '</p>'
    }).join('')

    resultElm.innerHTML = sliceElmPrag
    amountElm.value = ''
  }
})